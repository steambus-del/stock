...
for(const symbol of Object.keys(positions)){const stock=positions[symbol],quote=await getQuote(symbol);const currentPrice=quote.currentPrice,marketValue=stock.shares*currentPrice,costBasis=stock.costBasis,avgCost=stock.shares>0?costBasis/stock.shares:0,gainLoss=marketValue-costBasis,gainPercent=costBasis>0?gainLoss/costBasis*100:0;totalValue+=marketValue;totalCost+=costBasis;totalGain+=gainLoss;portfolioRows.push({symbol,shares:stock.shares,avgCost,currentPrice,dailyChange:quote.dailyChange,dailyChangePercent:quote.dailyChangePercent,marketValue,costBasis,gainLoss,gainPercent,priceRange: (quote.lowPrice > 0 && quote.highPrice > 0 ? formatMoney(quote.lowPrice) + " - " + formatMoney(quote.highPrice) : "--")})}

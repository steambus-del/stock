...
function beatText(v){if(!Number.isFinite(Number(v)))return"--";return(v>=0?"🟢 ":"🔴 ")+formatPercent(v)}
function beatClass(v){return !Number.isFinite(Number(v))?"muted":(v>=0?"gain":"loss")}
...
async function getVolumeInfo(symbol) {
    try {
        const from = unixDaysAgo(20);
        const to = unixNow();

        const url = `https://finnhub.io/api/v1/stock/candle?symbol=${encodeURIComponent(symbol)}&resolution=D&from=${from}&to=${to}&token=${API_KEY}`;
        ...
        const volumes = data.v.map(v => Number(v) || 0).filter(v => v > 0);
        if (volumes.length === 0) {
            return {
                todayVolume: 0,
                avg10Volume: 0
        ...
        const avg10Volume = last10.reduce((sum, v) => sum + v, 0) / last10.length;
        ...
        console.error("获取成交量失败:", symbol, error);
        return {
            todayVolume: 0,
            avg10Volume: 0
        };
...
function makeRangeDisplay(low, high, current) {
    ...
    if (low <= 0 || high <= 0 || high <= low || current <= 0) {
        return '<span class="range-missing">--</span>';
    }
    ...
    return `
        <div class="compact-range">
            ...
            <div class="compact-track">
                ...
                <div class="compact-marker" style="left:${percent}%;"></div>
            </div>
            <div class="compact-pos">${percent.toFixed(0)}%</div>
        </div>
    `;
}

async function loadPortfolio(){portfolioRows=[];const positions=calculatePositions();let totalValue=0,totalCost=0,totalGain=0;
for(const symbol of Object.keys(positions)){const stock=positions[symbol],quote=await getQuote(symbol),earn=await getEarningsInfo(symbol);const currentPrice=quote.currentPrice,marketValue=stock.shares*currentPrice,costBasis=stock.costBasis,avgCost=stock.shares>0?costBasis/stock.shares:0,gainLoss=marketValue-costBasis,gainPercent=costBasis>0?gainLoss/costBasis*100:0;totalValue+=marketValue;totalCost+=costBasis;totalGain+=gainLoss;portfolioRows.push({symbol,shares:stock.shares,avgCost,currentPrice,dailyChange:quote.dailyChange,dailyChangePercent:quote.dailyChangePercent,marketValue,costBasis,gainLoss,gainPercent,priceRange:makeRangeDisplay(quote.lowPrice, quote.highPrice, currentPrice),todayVolume:volumeInfo.todayVolume,avg10Volume:volumeInfo.avg10Volume,earningsDisplay:earn.display,earningsSort:earn.days,earningsClass:earn.className})}
